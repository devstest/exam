...
# from a yaml file (vagrant.yaml) in the root directory.
...
# Check the "nodes" element from vagrant.yaml for existence and completeness
def verify_nodes(nodes)
  # Make sure that at least one node is defined
  if !nodes || nodes.empty?
    error_msg = 'No nodes defined in vagrant.yaml'
...
# Convert the shell provisioner arguments from vagrant.yaml
...
# Verify that vagrant.yaml exists
root_dir = File.dirname(__FILE__)
vagrant_yaml_file = "#{root_dir}/vagrant.yaml"
error_msg = "#{vagrant_yaml_file} does not exist"
handle_error(error_msg) unless File.exists?(vagrant_yaml_file)

# Read box and node configs from vagrant.yaml
...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  # Define vagrant VMs for each node defined in vagrant.yaml
  nodes.each do |node_name, node_details|
    config.vm.define node_name do |node|
      ...
      #node.ssh.private_key_path = [".ssh/vagrant_id_rsa", "~/.vagrant.d/insecure_private_key"]
      #node.vm.provision "file", source: ".ssh/vagrant_id_rsa.pub", destination: "~/.ssh/authorized_keys"
      ...
      #       provider-specific in the .yaml files
